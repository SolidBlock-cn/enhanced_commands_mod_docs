...
使用 [`/testarg nbt_predicate <NBT 谓词> match <NBT>`](../../commands/testarg.md) 可用于判断指定的 NBT 是否符合指定的谓词。
...
!!! note "注意"
    和原版的 NBT 值的解析方法一样，如果不在引号内的文本能被解析为数字，则会被解析为数字；如果不在引号内的文本是 `true` 或 `false`，会被相应解析成 `1b` 和 `0b`；其他情况下，会被解析为字符串，例如 `abc` 和 `"abc"` 等价；如果文本内含有不能在引号外的字符，则解析失败，例如 `happy ghast` 不是有效的 NBT 值（因为含有空格），当然也不能作为 NBT 谓词，要表示这个字符串必须加引号，即 `"happy ghast"`。
