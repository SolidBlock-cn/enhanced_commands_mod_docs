title: 简单方块谓词
subtitle: 最基本的判断，但也绝不简单

# 简单方块谓词：最基本的判断，但也绝不简单

这是最基础的[方块谓词](index.md)。匹配一个方块的 ID，还可以匹配方块状态属性。

## 语法

`[@]<方块 ID>[方块状态属性]`

其中方块状态属性的语法请参见[属性方块谓词](../properties.md)。不同的是，由于方块的 ID 是明确指定的，因此在这种情况下，方块状态属性的名称和值的可接受值都可以确定。当遇到未知的方块状态属性名称或值时，会直接导致解析错误。
...
- [简单方块函数](../../block_function/simple.md)