...
这个[方块谓词](index.md)检测方块状态属性是否满足指定的条件。

对于方块 ID 直接接属性的语法（参见[简单方块谓词](simple.md)），属性和值是在解析时就已经完全确定，如果遇到无效的属性和值，会导致解析失败。对于其他的情况，属性的和值并不确定，需要在实际进行判断时，先判断方块是否有这一名称的属性，以及是否有这一名称的值，然后再对属性和值进行判断。
