# Argument types

Besides using vanilla argument types, this mod also adds some extra and more complicated **argument types**.

- [block function](block_function/index.md)
- [block predicate](block_predicate/index.md)
- [concentration type](concentration_type.md)
- [curve](curve/index.md)
- [direction](direction.md)
- [entity predicate](entity_predicate/index.md)
- [entity selector](entity_selector.md)
- [item function](item_function/index.md)
- [item predicate](item_predicate/index.md)
- [keyword arguments](keyword_args.md)
- [NBT concentration type](nbt_concentration_type.md)
- [NBT function](nbt_function/index.md)
- [NBT predicate](nbt_predicate/index.md)
- [NBT source](nbt_source.md)
- [NBT target](nbt_target.md)
- [position](pos.md)
- [region](region/index.md)