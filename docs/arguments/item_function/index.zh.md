...
**物品函数**（item function）可用于指定对物品堆叠（包括空堆叠）的修改。这个[参数类型](...md)既可用于给予物品，也可用于修改物品。在给予物品时，会先对空物品堆叠（从技术上讲，是空气物品）应用此函数。
...
像[复合方块函数](../block_function.md#复合方块函数)一样，物品函数也支持复合语法：
...
类似于原版的物品参数以及本模组中的[物品谓词](../item_predicate.md)，物品函数也支持指定物品组件列表。物品组件列表是在一对中括号内的，中括号直接添加在物品函数后，中括号前不能有空格。类似于原版，物品组件列表的每一项都支持如下语法：
...
- `component`：ID，物品组件类型。
- `function`：[NBT 函数](../nbt_function.md)。
- （当 `type` 为 `random`：）
- 无额外字段。
