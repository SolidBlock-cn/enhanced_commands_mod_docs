...
战利品表谓词是 Minecraft 的一项很强大的功能，支持对条件进行一些很复杂的判断。在 `/execute if predicate` 命令中，可以指定战利品表谓词的 ID，也可以直接以 NBT 的形式内联，但实体选择器中只支持指定 ID。不过别担心，借助此模组，你可以直接在实体选择器中以 NBT 形式内联战利品表谓词！

可以在等号后面加入感叹号（`!`）以将战利品表谓词取反，也可以使用 `#!js {condition: inverted, term: {...}}` 来取反。

在输入战利品表谓词的地方，直接输入 NBT，命令在编译时就可以直接解析，非常方便。例如：

- `#!js @e[predicate={condition: entity_properties, entity: this, predicate: {flags: {is_sneaking: true}}}]`：正在潜行的实体。
- `#!js @e[predicate={condition: location_check, predicate: {biome: badlands}}]`：正处于恶地生物群系中的实体。
