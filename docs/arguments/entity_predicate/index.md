...
- `type`：字符串，实体谓词的类型，[带有默认命名空间的 ID](../../glossaries/id_with_default_namespace.md)。具体可见的类型见下。

每个实体谓词都有一个类型，不同的类型会支持一些各自的字段（见对应具体实体选择器类型的页面）。以下为目前的所有实体谓词类型的 ID：

- [`advancements`](advancements.md)
- [`air`](air.md)
- [`air_max`](air.md)
- [`alive`](special/alive.md)（特殊类型）
- [`alternatives`](alternatives.md)
- [`baby`](baby.md)
- [`block_predicate`](block.md)
- [`block_predicates`](block.md)
- [`box`](box.md)
- [`collector`](special/collector.md)（特殊类型）
- [`distance`](special/distance.md)（特殊类型）
- [`effect`](effect.md)
- [`effects`](effect.md)
- [`empty`](empty.md)
...
- [`level`](level.md)
- [`local_world`](special/local_world.md)（特殊类型）
...
- [`selector`](special/selector.md)（特殊类型）
