...
> 注意：如果方块谓词是 [NBT 谓词](../block_predicate/nbt.md)，请注意给 NBT 谓词加上括号或者星号，以免被解析为第二种语法。例如，`[block={Inventory: []}]` 是错误的，应当写成 `[block=({Inventory: []})]` 或 `[block=*{Inventory: []}]`。
...
- `predicates`：列表。
    - 列表中的一个元素。
        - `pos`：[坐标](../pos.md)。
        - `block`：[方块谓词](../block_predicate/index.md)。