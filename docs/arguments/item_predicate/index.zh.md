...
**物品谓词**（item predicate）是一种[参数类型](...md)，可用于测试一个物品堆叠（item stack）是否满足条件。一个物品堆叠可能匹配此谓词（测试通过），也可能不匹配此谓词（测试失败）。
...
多个物品谓词可以进行复合，符号 `|` 取交集，符号 `&` 取并集。`!` 可用于将一个谓词取反。具体用法类似于[方块谓词#复合方块谓词](../block_predicate.md#复合方块谓词)，本页不再赘述。
