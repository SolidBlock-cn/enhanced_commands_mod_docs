...
- `/moveblocks sphere(5)`：将以执行源所在的坐标为中心、半径 5 个方块的球体内的方块向执行源的方向移动 1 格。
- `//moveblocks`：将玩家的活动区域内的方块向执行源的前方方向移动 1 格。
- `//moveblocks 5`：将玩家的活动区域内的方块向执行源的前方方向移动 5 格。
- `//moveblocks 5 east`：将玩家的活动区域内的方块向东移动 5 格。
- `//moveblocks 5 back_horizontal`：将玩家的活动区域内的方块向执行源的水平方向的朝向的后方移动 5 格。
- `//moveblocks 1 2 3`：将玩家的活动区域按照(1, 2, 3)的向量移动。
- `/moveblocks sphere(5) 1 front select=true`：移动半径为 5 个方块的球体内的方块，并将玩家的移动区域设为移动后的球体。
- `//moveblocks 1 front select=true`：移动玩家的活动区域内的方块，并将玩家的活动区域也移动到移动后的位置。
- `//moveblocks 1 front affect_entities=@e`：移动玩家的活动区域内的方块，以及在该区域内的所有实体（包括玩家，如果玩家也在该区域内）。
- `//moveblocks 1 front transform_only=!air`：移动玩家的活动区域内的所有除空气以外的方块。